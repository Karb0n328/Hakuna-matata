(() => {
  ...
  const stateOwner=()=>localStorage.getItem('hakuna.cloud.localOwner')||'';
  function readState(){const owner=stateOwner();return new Promise((resolve,reject)=>{const tx=db.transaction('app','readonly');const req=tx.objectStore('app').get(STATE_KEY);req.onsuccess=()=>{if(owner!==stateOwner())return reject(new Error('Hesap değişti. Mata’yı yeniden aç.'));const state=req.result||{};snapshots.set(state,{raw:JSON.stringify(req.result||null),owner});resolve(state);};req.onerror=()=>reject(req.error);});}
  // Compare and write within one IDB transaction: never overwrite a newer app/cloud state.
  function writeState(state){return new Promise((resolve,reject)=>{const baseline=snapshots.get(state);if(!baseline)return reject(new Error('Kayıt yeniden okunmalı.'));const tx=db.transaction('app','readwrite'),store=tx.objectStore('app');let failure;const req=store.get(STATE_KEY);req.onsuccess=()=>{if(baseline.owner!==stateOwner()||baseline.raw!==JSON.stringify(req.result||null)){failure=new Error('Veriler veya hesap değişti. İsteğini yeniden gönder; hiçbir işlem uygulanmadı.');tx.abort();return;}store.put(state,STATE_KEY);};tx.oncomplete=()=>{snapshots.set(state,{raw:JSON.stringify(state),owner:baseline.owner});resolve();};tx.onerror=()=>reject(failure||tx.error||new Error('Kayıt yazılamadı.'));tx.onabort=()=>reject(failure||tx.error||new Error('Kayıt iptal edildi.'));});}
